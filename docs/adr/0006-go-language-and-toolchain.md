...
- Use the official MCP Go SDK for MCP protocol boundaries.
- Use `fsnotify` for the native watcher adapter and the Moby client for Docker API integration.
- Treat `go.mod` as the authority on which version of each is in use. This decision selects libraries, not revisions; naming a revision here would make an accepted record go stale on every routine bump.

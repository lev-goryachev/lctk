...
Activation uses a candidate-and-swap transaction. LCTK starts the selected image under a temporary managed name on the runtime network, performs health, backend, and embedding checks, and records the current container's complete user-defined network attachments and aliases. Only then does it replace the final name and reconnect the exact recorded project networks with their aliases. Any swap or reconnection failure removes the candidate, recreates the previous pinned distribution, reconnects its saved networks, proves its health, and leaves the previous selection active. Unexpected inspect output is fatal; it is never treated as absence.

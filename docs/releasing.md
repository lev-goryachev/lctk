...
Before publishing, `scripts/build-local-rc.ps1` creates unsigned `.artifacts/LCTK-Setup-local-RC.exe` and `.artifacts/LCTK-Uninstall-local-RC.exe` files from the current worktree. It verifies the published template manifest, replaces only the setup/core/launcher identities, signs the candidate manifest with an ephemeral local Ed25519 key, and appends the same candidate set to both native bootstrapper modes. The setup candidate opens the complete installation transaction. The recovery candidate opens the new uninstaller directly so a partially removed older installation can be cleaned without reinstalling its runtime or replacing files outside the repository. Neither mode creates a tag or GitHub Release. Numeric-loopback HTTP is allowed only for the appended artifacts; signature, length, and digest checks remain mandatory. These are local acceptance artifacts, not publication artifacts.

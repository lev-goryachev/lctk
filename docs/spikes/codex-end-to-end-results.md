...
| `file_outline_through_client` | **pass.** Declarations with kinds and extents, a field reported inside its type and a constant inside the method declaring it, and `"precision":"syntax"` stated in the answer. Containment is the part worth checking here: neither of those two declarations says in its own syntax where it lives. |
...
| `unsupported_language_refused` | **pass.** `LANGUAGE_UNSUPPORTED: Outlines are not available for ".txt"; this build understands c, cpp, go, javascript, python, rust, tsx, typescript. Use exact_search on this file instead; project_info lists the languages this project can outline.` An empty outline would have read as "this file declares nothing". |

...
**Status:** implementation complete; clean-host certification is not yet claimed. ADR-0023 replaces the Windows end-user Docker/Compose dependency with a private Podman `5.8.2` client and explicitly named WSL2 machine. ADR-0024 replaces the temporary browser setup with one native Win32 window that performs a read-only host/component/disk plan, lets the user select the installation and runtime-data locations, applies UAC-gated WSL enablement with RunOnce reboot continuation, downloads immutable client and machine artifacts, safely and idempotently extracts the portable client, runs explicit project runtime operations and the host bootstrap self-test, and registers sign-in, Start menu, and uninstall. Opening LCTK without arguments reconnects to or starts the daemon and opens a fresh authenticated Admin UI session; the UI can register and start projects without a shell. Schema-2 release manifests fail closed unless the complete Windows setup, launcher, core, runtime, image, and model set is present. A local Windows 10 build 19045 run recovered a partial installation through the native setup and reached a healthy installed daemon. Final certification still requires executing the unsigned artifact through the expected Windows warning on a clean Windows 10 22H2 amd64 host with no Docker Desktop, Go, or Git.
